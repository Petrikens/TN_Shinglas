const roofHarmonySlider = document.getElementById("roof_harmony_slider");
...
const harmonySlider = new HarmonySlider(roofHarmonySlider);

harmonySlider.createSliderItems();

harmonySlider.arrows.addEventListener("click", function (event) {
  let target = event.target;

  if (target.classList.contains("next")) {
    harmonySlider.slideIndex++;
    harmonySlider.activeItem(harmonySlider.slideIndex);
    if (harmonySlider.slideIndex > harmonySlider.maxIndex) {
      harmonySlider.slideIndex = 0;
      harmonySlider.activeItem(harmonySlider.slideIndex);
    }
  } else if (target.classList.contains("prev")) {
    if (harmonySlider.slideIndex > 0) {
      harmonySlider.slideIndex--;
      harmonySlider.activeItem(harmonySlider.slideIndex);
    } else if (harmonySlider.slideIndex === 0) {
      harmonySlider.slideIndex = harmonySlider.maxIndex;
      harmonySlider.activeItem(harmonySlider.slideIndex);
    }
  } else {
    return;
